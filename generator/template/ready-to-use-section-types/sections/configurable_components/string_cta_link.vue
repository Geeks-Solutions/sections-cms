<template>
  <div class="w-full">
    <div id="cta_link" class="flex flex-col items-start justify-start">
      <label class="mr-4 font-bold">{{ $t('ctaLink') }}</label>
      <input
        v-model="cta_link"
        type="text"
        :placeholder="$t('ctaLink')"
        :class="inputStyle"
...
export default {
  ...
  watch: {
    reference: {
      handler(value) {
        ...
        if (
          this.configurableReference.optionsData.cta_link &&
          typeof this.configurableReference.optionsData.cta_link === 'string'
        ) {
  ...
  methods: {
    changeFieldValue() {
      this.configurableReference.optionsData.cta_link = this.cta_link
      this.configurableReference.options[0].cta_link = this.cta_link
    },
  },
}
</script>
