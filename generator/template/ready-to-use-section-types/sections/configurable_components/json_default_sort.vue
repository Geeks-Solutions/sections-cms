...
export default {
    ...
    reference: {
      handler(value) {
        ...
        if (this.configurableReference.optionsData.default_sort) {
          this.default_sort = JSON.stringify(
  ...
  methods: {
    changeFieldValue() {
      try {
        this.configurableReference.optionsData.default_sort = JSON.parse(
          this.default_sort,
        )
        this.configurableReference.options[0].default_sort = JSON.parse(
          this.default_sort,
        )
      } catch {
        this.configurableReference.optionsData.default_sort = {}
