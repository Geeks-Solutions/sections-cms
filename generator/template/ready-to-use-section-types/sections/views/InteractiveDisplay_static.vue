  ...
  <div v-if="settings" class="interactive-display">
    ...
    >
      <div class="carousel-inner h-full">
        <div
          v-for="(block, index) in migratedSettings"
          :key="`carousel-media-block-${index}`"
        ...
        >
          ...
          <div
            v-if="
              (block.title[lang] || block.text[lang] || block.cta[lang]) &&
              index === currentIndex
            "
            class="absolute top-0 w-full h-full"
          >
            <div
              class="flex flex-col h-full justify-center items-center space-y-10"
            >
              <div class="flex flex-col gap-2 md:gap-4">
                <h2 v-if="block.title[lang]">
                  <gWysiwygContent
                    tag="span"
                    :wrapper-classes="block.titleClasses"
                    :classes="`text-center overflow-hidden p-0`"
                    :html-content="block.title[lang]"
                  />
                </h2>
                <p v-if="block.text[lang]">
                  <gWysiwygContent
                    tag="span"
                    :wrapper-classes="block.textClasses"
                    :classes="`text-center p-0`"
                    :html-content="block.text[lang]"
                  />
                </p>
              </div>
              <global-link
                v-if="block.cta && block.cta[lang]"
                :link="
                  !block.sectionsPage ||
                  (block.sectionsPage &&
                    (block.sectionsPage[lang] === 'other' ||
                      !block.sectionsPage[lang]))
                    ? block.ctaLink
                    : block.sectionsPage && block.sectionsPage[lang]
                      ? {
                          ...block.sectionsPage,
                          en: '/' + block.sectionsPage.en,
                          fr: '/' + block.sectionsPage.fr,
                        }
                      : '#'
                "
                :lang="lang"
                :default-lang="defaultLang"
                :form-link-target="block.ctaLinkTarget"
              >
      ...
      <div
        class="carousel-dots absolute bottom-0 left-0 right-0 flex justify-center pb-4"
      >
        <span
          v-for="(item, index) in migratedSettings"
          :key="index"
          :class="
            migratedSettings.length > 1
    ...
    <div v-else class="w-full relative video-wrapper">
      ...
      <div
        class="mobileHidden"
        :style="
          settings[0].aspectRatio
            ? `aspect-ratio: ${settings[0].aspectRatio}`
            : 'aspect-ratio: 16/9'
        "
      >
        <iframe
      ...
      <div
        class="md:hidden"
        :style="
          settings[0].mobileAspectRatio
            ? `aspect-ratio: ${settings[0].mobileAspectRatio}`
            : 'aspect-ratio: 9/16'
        "
      >
        <iframe
          :src="computedMobileVideoUrl"
          style="position: absolute; top: 0; left: 0; width: 100%; height: 100%"
          frameborder="0"
          webkitallowfullscreen=""
          mozallowfullscreen=""
      ...
      <div
        v-if="
          (settings[0].videoTitle && settings[0].videoTitle[lang]) ||
          (settings[0].videoText && settings[0].videoText[lang]) ||
          (settings[0].videoCta && settings[0].videoCta[lang])
        "
        ...
        >
          <div class="flex flex-col gap-2 md:gap-4 pointer-events-auto">
            <h2 v-if="settings[0].videoTitle[lang]">
              <gWysiwygContent
                tag="span"
                :wrapper-classes="settings[0].videoTitleClasses"
                :classes="`text-center overflow-hidden p-0`"
                :html-content="settings[0].videoTitle[lang]"
              />
            </h2>
            <p v-if="settings[0].videoText[lang]">
              <gWysiwygContent
                tag="span"
                :wrapper-classes="settings[0].videoTextClasses"
                :classes="`text-center p-0`"
                :html-content="settings[0].videoText[lang]"
              />
            </p>
          </div>
          <global-link
            v-if="settings[0].videoCta && settings[0].videoCta[lang]"
            :link="
              !settings[0].sectionsPage ||
              (settings[0].sectionsPage &&
                (settings[0].sectionsPage[lang] === 'other' ||
                  !settings[0].sectionsPage[lang]))
                ? settings[0].videoCtaLink
                : settings[0].sectionsPage && settings[0].sectionsPage[lang]
                  ? {
                      ...settings[0].sectionsPage,
                      en: '/' + settings[0].sectionsPage.en,
                      fr: '/' + settings[0].sectionsPage.fr,
                    }
                  : '#'
            "
            :lang="lang"
            :default-lang="defaultLang"
            :form-link-target="settings[0].videoCtaLinkTarget"
  ...
  computed: {
    ...
    migratedSettings() {
      if (
        this.settings &&
        this.settings[0] &&
        this.settings[0].carousels === undefined
      ) {
        return this.section.settings
    ...
    computedMobileVideoUrl() {
      if (
        this.settings &&
        this.settings[0] &&
        !this.settings[0].mobileVideoLink &&
        this.settings[0].videoLink &&
        this.isYouTubeLink(this.settings[0].videoLink)
      ) {
        const v = this.settings[0]
        ...
        this.settings[0] &&
        this.settings[0].mobileVideoLink &&
        this.isYouTubeLink(this.settings[0].mobileVideoLink)
      ) {
        const v = this.settings[0]
        try {
        ...
        return this.settings[0].mobileVideoLink
          ? this.settings[0].mobileVideoLink
          : this.settings[0].videoLink
    },
  },
