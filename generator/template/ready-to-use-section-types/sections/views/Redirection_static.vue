<template>
  <div v-if="settings">
    <gWysiwygContent
      tag="p"
      :wrapper-classes="settings.classes"
      :classes="`mobileTitleStyle mt-4 md:mx-4 title w-full`"
      :html-content="settings[lang].message"
    />
  </div>
</template>
...
export default {
  ...
  mounted() {
    if (
      this.settings &&
      this.settings.timeToRedirect &&
      this.settings[this.lang].redirectionUrl &&
