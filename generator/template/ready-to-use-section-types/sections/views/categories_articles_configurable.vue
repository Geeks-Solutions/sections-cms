<template>
  ...
  >
    <div v-if="sectionRenderData">
      <BlogsArticles
        :section-render-data="sectionRenderData"
        :section-render-settings="sectionRenderSettings"
        :total-pages="totalPages"
        :lang="lang"
        :default-lang="defaultLang"
        list-type="listing"
        @page-changed="(offset) => pageChanged(offset)"
      />
    </div>
  </div>
</template>
...
export default {
  ...
  computed: {
    sectionRenderData() {
      if (
        this.section.render_data &&
        this.section.render_data[0] &&
        this.section.render_data[0].data
      ) {
    ...
    sectionRenderSettings() {
      if (
        this.section.render_data &&
        this.section.render_data[0] &&
        this.section.render_data[0].settings
      ) {
  ...
  methods: {
    pageChanged(offset) {
      ...
      this.$emit('refresh-section', {
        qs: {
          categories_titles: this.categoriesTitles.includes(',')
            ? this.categoriesTitles.split(',')
            : this.categoriesTitles.length > 0
