<template>
  <div class="p-4">
        ...
        <template #default="{ object, idx }">
          ...
          <div class="mb-4 mt-4">
            <LazyMediasUploadMedia
              :media-label="$t('RestaurantMenu.categoryIcon')"
              :upload-text="$t('RestaurantMenu.uploadIcon')"
              :change-text="$t('RestaurantMenu.changeIcon')"
            ...
            <span class="flex text-xs text-Gray_800">{{
              $t('RestaurantMenu.categoryIconDesc')
            }}</span>
          </div>

          <div class="flex flex-col items-start justify-start mt-4">
            <label class="mr-4 font-medium">{{
              $t('RestaurantMenu.categoryDescription')
            }}</label>
            <textarea
          ...
          <div class="flex flex-col items-start justify-start mt-4">
            ...
            <span class="text-xs text-Gray_800">{{
              $t('RestaurantMenu.cssClassesDesc')
            }}</span>
            <input
    ...
    <div id="menu-items" class="flex flex-col mt-8 pt-8 border-t">
      ...
      <div v-if="selectedCategoryId" class="w-full">
        <LazySectionsFormsFieldSets
          ...
          @array-updated="
            (data) => updateMenuItemsForCategory(selectedCategoryId, data)
          "
        ...
        >
          <template #default="{ object }">
            ...
            <div class="flex flex-col items-start justify-start mt-4">
              <label class="mr-4 font-medium"
                >{{ $t('RestaurantMenu.itemName') }}*</label
              >
              <input
                v-model="object.name[selectedLang]"
                type="text"
              ...
              <span
                v-show="
                  getErrorForMenuItem(object.id, 'name') === true &&
                  selectedLang === defaultLang
                "
                class="text-error text-sm pt-2 pl-2"
                >{{ $t('RestaurantMenu.requiredField') }}</span
            ...
            <div class="flex flex-col items-start justify-start mt-4">
              <label class="mr-4 font-medium">{{
                $t('RestaurantMenu.itemDescription')
              }}</label>
              <textarea
                v-model="object.description[selectedLang]"
                type="text"
            ...
            <div class="flex flex-col items-start justify-start mt-4">
              <label class="mr-4 font-medium"
                >{{ $t('RestaurantMenu.itemPrice') }}*</label
              >
              <input
              ...
              />
              <span
                v-show="getErrorForMenuItem(object.id, 'price') === true"
                class="text-error text-sm pt-2 pl-2"
                >{{ $t('RestaurantMenu.requiredField') }}</span
              >
            </div>
            ...
            <div class="mb-4 mt-4">
              <LazyMediasUploadMedia
                :media-label="$t('RestaurantMenu.itemImage')"
                :upload-text="$t('RestaurantMenu.uploadMedia')"
                :change-text="$t('RestaurantMenu.changeMedia')"
                :seo-tag="$t('RestaurantMenu.seoTag')"
                :media="
              ...
              />
              <span class="flex text-xs text-Gray_800">{{
                $t('RestaurantMenu.itemImageDesc')
              }}</span>
            </div>
            ...
            <div class="flex flex-col items-start justify-start mt-4">
              ...
              <div class="flex items-center">
                ...
                />
                <span class="text-xs text-Gray_800">{{
                  $t('RestaurantMenu.featuredItemDesc')
                }}</span>
              </div>
            </div>
        ...
        <div
          class="add-button underline cursor-pointer mt-2"
          @click="addMenuItem(selectedCategoryId)"
        >
    ...
    <div class="flex flex-col items-start justify-start mt-8 pt-8 border-t">
      <h3 class="text-lg font-semibold mb-4">
        {{ $t('RestaurantMenu.generalSettings') }}
      </h3>
      ...
      <div class="mb-6">
        <LazyMediasUploadMedia
          :media-label="
            $t('RestaurantMenu.restaurantLogo') || 'Restaurant Logo'
          "
          :upload-text="$t('RestaurantMenu.uploadLogo') || 'Upload Logo'"
          :change-text="$t('RestaurantMenu.changeLogo') || 'Change Logo'"
          :seo-tag="$t('RestaurantMenu.seoTag')"
          :media="
        ...
        />
        <span class="flex text-xs text-Gray_800">{{
          $t('RestaurantMenu.logoDesc')
        }}</span>
      </div>

      <div class="flex flex-col items-start justify-start mt-4">
        <label class="mr-4 font-medium">{{
          $t('RestaurantMenu.menuTitle')
        }}</label>
        <input
          v-model="settings[0].menuTitle[selectedLang]"
          type="text"
      ...
      <div class="flex flex-col items-start justify-start mt-4">
        ...
        <input
          v-model="settings[0].menuSubtitle[selectedLang]"
          type="text"
      ...
      <div class="flex flex-col items-start justify-start mt-8 pt-8 border-t">
        ...
        >
          ...
          <input
            ...
            :class="[
              sectionsStyle.input,
              errors.generalSettings.taxRate ? 'error' : '',
            ]"
          />
          <span
          ...
          <div class="flex flex-col items-start justify-start">
            ...
            <div v-if="settings[0].showWhatsApp" class="w-full">
              ...
              <input
                ...
                type="text"
                placeholder="Hello! I would like to reserve a table."
                :class="sectionsStyle.input"
              />
            </div>
          </div>
...
const updateMediasArray = (media) => {
  ...
  const existingIndex = settings.value[0].medias.findIndex(
...
const removeFromMediasArray = (mediaId) => {
  if (!mediaId) return
  settings.value[0].medias = settings.value[0].medias.filter(
...
const removeCategoryIcon = (categoryId) => {
  const categoryIndex = settings.value[0].categories.findIndex(
...
const removeCategory = (idx) => {
  ...
  settings.value[0].categories = settings.value[0].categories.filter(
    (ct, i) => idx !== i,
  )
...
const removeMenuItem = (itemId) => {
  const idx = settings.value[0].menuItems.findIndex(
    (item) => item.id === itemId,
  )
  if (idx === -1) return

  const mediaId =
    settings.value[0].menuItems[idx].image &&
    settings.value[0].menuItems[idx].image.media_id
  if (mediaId) {
    removeFromMediasArray(mediaId)
  }
...
const getCategoryOptions = () => {
  if (
    !settings.value[0].categories ||
    settings.value[0].categories.length === 0
  )
    return []
...
const getMenuItemsByCategory = (categoryId) => {
  ...
  return settings.value[0].menuItems.filter(
    (item) => item.categoryId === categoryId,
  )
}
...
const updateMenuItemsForCategory = (categoryId, updatedItems) => {
  ...
  const otherItems = settings.value[0].menuItems.filter(
    (item) => item.categoryId !== categoryId,
  )
...
const validate = () => {
  ...
  if (
    settings.value[0].enableTax &&
    !settings.value[0].taxRate &&
    settings.value[0].taxRate !== 0
  ) {
  ...
  if (
    settings.value[0].logo &&
    (Object.keys(settings.value[0].logo).length === 0 ||
      !settings.value[0].logo.media_id ||
      !settings.value[0].logo.url)
  ) {
...
watch(
  ...
  (mediaObject) => {
    ...
    if (currentMediaType.value === 'logo') {
      ...
      const itemIndex = settings.value[0].menuItems.findIndex(
        (item) => item.id === currentMediaItemId.value,
      )
      if (itemIndex !== -1) {
...
onMounted(() => {
  ...
  props.locales.forEach((locale) => {
    if (!settings.value[0].whatsappMessage[locale]) {
      settings.value[0].whatsappMessage[locale] =
