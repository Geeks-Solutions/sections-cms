      ...
      <div class="my-4 ml-6">
        ...
        <div class="select-style-chooser w-[344px]">
          <gAutoComplete
            ...
            :filter-options="[{key: '16/9', value: '16:9'}, {key: '9/16', value: '9:16'}]"
            :filter-searchable="false"
            :close-on-select="true"
            :filter-clearable="true"
            :track-by="'key'"
            @itemSelected="(val) => {settings[0].aspectRatio = val;}"
          >
          </gAutoComplete>
        </div>
      </div>
...
<!--    Mobile video start  -->
    <div class="flex flex-col items-start justify-start mt-8 ml-2">
      <label :class="sectionsStyle.fieldLabel">{{ $t("forms.video") + '(Mobile)' }}</label>
      <input
      ...
      <div class="my-4 ml-6">
        ...
        <div class="select-style-chooser w-[344px]">
          <gAutoComplete
            ...
            :filter-clearable="true"
            :track-by="'key'"
            @itemSelected="(val) => {settings[0].mobileAspectRatio = val;}"
          >
          </gAutoComplete>
        </div>
      </div>
...
<!--    Mobile video end  -->
    ...
    <div id="containerHeight" class="flex flex-col items-start justify-start mt-8 ml-2">
...
export default {
  ...
  methods: {
    ...
    removeMedia(idx, key) {
      try {
        const mediaIndex = this.settings[0].medias.find(md => md.media_id === this.settings[0].carousels[idx][key].media_id)
    ...
    removeCarouselBlock(idx) {
      if (this.settings[0].carousels.length > 1) {
        this.settings[0].carousels = this.settings[0].carousels.filter((img, i) => idx !== i);
      }
    },
    validate() {
      try {
      ...
      if (this.settings[0].videoLink) {
		valid = true;
	  } else if (!this.settings[0].videoLink && (!this.settings[0].carousels[0].media || !this.settings[0].carousels[0].media.url) && (!this.settings[0].carousels[0].mediaMobile || !this.settings[0].carousels[0].mediaMobile.url)) {
        if (!this.settings[0].carousels[0].media || !this.settings[0].carousels[0].media.url) {
          this.errors.media = true;
        } else if (!this.settings[0].carousels[0].mediaMobile || !this.settings[0].carousels[0].mediaMobile.url) {
          this.errors.mediaMobile = true;
        }
        this.errors.videoLink = true;
        valid = false;
      } else if (this.settings[0].carousels[0].media && this.settings[0].carousels[0].media.url) {
        if (!this.settings[0].carousels[0].mediaMobile || !this.settings[0].carousels[0].mediaMobile.url) {
          this.errors.mediaMobile = true;
          valid = false;
        }
      }
