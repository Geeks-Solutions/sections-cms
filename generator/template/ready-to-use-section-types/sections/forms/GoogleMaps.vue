  ...
  <div class="p-4">
    <div :id="`mapApiKey`" class="flex flex-col items-start justify-start mt-8">
      ...
      <p class="text-start pb-2">
        <span class="text-sm">{{ $t('forms.apiKeyDesc') }}</span>
        <a
          href="https://developers.google.com/maps/documentation/javascript/get-api-key"
          target="_blank"
          class="underline text-sm text-Blue"
    ...
    <div class="my-4">
      ...
      <div class="select-style-chooser">
        <gAutoComplete
          ...
          :filter-options="[
            { key: 'fit_pins', value: $t('forms.fitPins') },
            { key: '1', value: '1' },
            { key: '2', value: '2' },
            { key: '3', value: '3' },
            { key: '4', value: '4' },
    ...
    <div id="pins" class="flex flex-col mt-4">
      <LazySectionsFormsFieldSets
        :array-data-pop="settings[0].pins"
        :fieldset-group="'pins'"
        :legend-label="$t('forms.pin')"
        @array-updated="(data) => (settings[0]['pins'] = data)"
        @remove-fieldset="(object, idx) => removePinBlock(idx)"
      >
          ...
          <div>
            ...
            >
              ...
              <input
                ...
                :class="sectionsStyle.input"
                @input="
                  settings[0].pins[idx] = { ...object, type: object.type }
                "
              />
              <span
                v-show="errors[`pin-type-${idx}`] === true"
                class="text-error text-sm pt-2 pl-2"
                >{{ $t('forms.requiredField') }}</span
              >
            </div>
      ...
      <div class="add-button underline cursor-pointer mt-2" @click="addPin()">
    ...
    <div id="addresses" class="flex flex-col mt-4">
      <LazySectionsFormsFieldSets
        :array-data-pop="settings[0].addresses"
        :fieldset-group="'addresses'"
        :legend-label="$t('forms.address')"
        @array-updated="(data) => (settings[0]['addresses'] = data)"
        @remove-fieldset="(object, idx) => removeAddressBlock(idx)"
      >
        <template #default="{ object, idx }">
          <div>
            <div
            ...
            >
                  ...
                  <gAutoComplete
                    ...
                    :filter-options="[
                      ...
                      ...settings[0].pins
                        .filter(
                          (pin) =>
                            pin.type !== '' && pin.name[selectedLang] !== '',
                        )
                        .map((pin) => {
                          return {
                            key: pin.type,
                            option: pin.name[selectedLang],
                            value: pin.name[selectedLang],
                            selected: pin.name[selectedLang],
                            image:
                              pin.media && pin.media.url ? pin.media.url : '',
                            label:
                              pin.media && pin.media.url ? pin.media.url : '',
                          }
                        }),
              ...
              <span
                v-show="errors[`address-type-${idx}`] === true"
                class="text-error text-sm pt-2 pl-2"
                >{{ $t('forms.requiredField') }}</span
              >
            </div>
            ...
            >
              ...
              <span
                v-show="
                  errors[`address-street-${idx}`] === true &&
                  selectedLang === defaultLang
                "
                class="text-error text-sm pt-2 pl-2"
                >{{ $t('forms.requiredField') }}</span
            ...
            >
              ...
              <span
                v-show="
                  errors[`address-country-${idx}`] === true &&
                  selectedLang === defaultLang
                "
                class="text-error text-sm pt-2 pl-2"
                >{{ $t('forms.requiredField') }}</span
            ...
            >
              ...
              />
              <span
                v-show="errors[`address-lat-${idx}`] === true"
                class="text-error text-sm pt-2 pl-2"
                >{{ $t('forms.requiredField') }}</span
              >
            </div>
            ...
            >
              ...
              />
              <span
                v-show="errors[`address-lng-${idx}`] === true"
                class="text-error text-sm pt-2 pl-2"
                >{{ $t('forms.requiredField') }}</span
              >
            </div>
...
export default {
    ...
    selectedMedia(mediaObject) {
      ...
      if (this.settings[0].pins[this.mediaFieldIndex].media.media_id) {
        const mediaIndex = this.settings[0].medias.findIndex(
          (media) =>
            media.media_id ===
            this.settings[0].pins[this.mediaFieldIndex].media.media_id,
        )
        if (mediaIndex !== -1) {
  ...
  methods: {
    ...
    confirmLocation() {
      if (this.marker) {
        this.settings[0].addresses[this.selectedAddressIndex].lat =
          this.marker[0]
        this.settings[0].addresses[this.selectedAddressIndex].lng =
          this.marker[1]
      }
      this.marker = null
    ...
    uploadMedia(idx) {
      ...
      this.$emit(
        'openMediaModal',
        Object.keys(this.settings[0].pins[idx].media).length > 0
          ? this.settings[0].pins[idx].media.media_id
          : null,
      )
    },
    removeMedia(idx) {
      const removeAtIndex = this.settings[0].medias.findIndex(
        (media) => media.media_id === this.settings[0].pins[idx].media.media_id,
      )
    ...
    removeAddressBlock(idx) {
      if (this.settings[0].addresses.length > 1) {
        this.settings[0].addresses = this.settings[0].addresses.filter(
          (blc, i) => idx !== i,
        )
      }
    },
    ...
    removePinBlock(idx) {
      this.settings[0].pins = this.settings[0].pins.filter(
        (blc, i) => idx !== i,
      )
    },
    validate() {
      if (Array.isArray(this.settings)) {
        this.settings.forEach((ob, index) => {
          if (ob.wysiwygLang) {
            if (
              !JSON.stringify(this.settings[0]).includes(
