  ...
  <div>
    ...
    <div id="media" class="flex flex-col mt-4">
      <LazySectionsFormsFieldSets
        :array-data-pop="settings[0].features"
        :fieldset-group="'menu'"
        :legend-label="$t('forms.feature')"
        @array-updated="(data) => (settings[0]['features'] = data)"
        @remove-fieldset="(object, idx) => removeFeatureBlock(idx)"
      >
...
export default {
  ...
  watch: {
    ...
    selectedMedia(mediaObject) {
      ...
      this.settings[0].features[this.selectedMediaIndex][
  ...
  methods: {
    ...
    removeMedia(idx, key) {
      try {
        const mediaIndex = this.settings[0].medias.find(
          (md) => md.media_id === this.settings[0].features[idx][key].media_id,
        )
    ...
    removeFeatureBlock(idx) {
      if (this.settings[0].features.length > 1) {
        this.settings[0].features = this.settings[0].features.filter(
          (img, i) => idx !== i,
        )
      }
    },
