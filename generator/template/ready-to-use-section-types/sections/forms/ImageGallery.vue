<template>
  <div>
    ...
    <div id="selectedLayout" class="flex flex-col items-start justify-start mt-8">
      <label for="dropdown" class="block mb-2 text-sm font-medium text-gray-700">{{ $t('forms.selectedLayout') }}</label>
      <select
        id="dropdown"
      ...
      >
        <option value="verticalMasonry">{{ $t('forms.verticalMasonry') }}</option>
        <option value="horizontalMasonry">{{ $t('forms.horizontalMasonry') }}</option>
      ...
      <span v-if="errors.selectedLayout === true" class="flex text-error text-sm pt-2 pl-2">{{ $t('forms.requiredField') }}</span>
    </div>

    <div v-if="settings[0].selectedLayout === 'slider' || settings[0].selectedLayout === 'thumbnails'" id="containerHeight" class="flex flex-col items-start justify-start mt-8">
      <label for="dropdown" class="block mb-2 text-sm font-medium text-gray-700">{{ $t("forms.containerHeight") }}</label>
      <input
    ...
    <div v-if="settings[0].selectedLayout === 'slider' || settings[0].selectedLayout === 'thumbnails'" id="imageFit" class="flex flex-col items-start justify-start mt-8">
      <label for="dropdown" class="block mb-2 text-sm font-medium text-gray-700">{{ $t('forms.imageFit') }}</label>
      <select
        id="dropdown"
...
export default {
  ...
  methods: {
    ...
    removeMedia(idx, key) {
      try {
        const mediaIndex = this.settings[0].medias.find(md => md.media_id === this.settings[0].gallery[idx][key].media_id)
    ...
    removeGalleryBlock(idx) {
      if (this.settings[0].gallery.length > 1) {
        this.settings[0].gallery = this.settings[0].gallery.filter((img, i) => idx !== i);
      }
    },
    validate() {
      ...
      if ((!this.settings[0].gallery[0].media || !this.settings[0].gallery[0].media.url) && (!this.settings[0].gallery[0].mediaMobile || !this.settings[0].gallery[0].mediaMobile.url)) {
        if (!this.settings[0].gallery[0].media || !this.settings[0].gallery[0].media.url) {
          this.errors.media = true;
        } else if (!this.settings[0].gallery[0].mediaMobile || !this.settings[0].gallery[0].mediaMobile.url) {
          this.errors.mediaMobile = true;
        }
        valid = false;
      } else if (this.settings[0].gallery[0].media && this.settings[0].gallery[0].media.url) {
        if (!this.settings[0].gallery[0].mediaMobile || !this.settings[0].gallery[0].mediaMobile.url) {
          this.errors.mediaMobile = true;
          valid = false;
        }
      }
