<template>
  <div>
    <div id="message" class="flex flex-col items-start justify-start mt-8">
      ...
      <LazyEditorWysiwyg
        :html="settings[0][selectedLang].message"
        :css-classes-prop="settings[0].classes"
        @cssClassesChanged="(v) => (settings[0]['classes'] = v)"
        @wysiwygMedia="wysiwygMediaAdded"
        @settingsUpdate="updateMessageDescription"
      />
      <span
        v-show="errors.message && selectedLang === defaultLang"
    ...
    <div class="flex flex-col items-start justify-start mt-8">
      ...
      <div class="flex flex-row items-center gap-1 pb-2">
        ...
        <span class="mr-4 text-xs font-bold text-start">{{
          $t('forms.redirectionWarning')
        }}</span>
      </div>
      <input
      ...
      />
      <span
        v-show="errors.redirectionUrl && selectedLang === defaultLang"
        class="text-error text-sm pt-2 pl-2"
        >{{ $t('forms.requiredField') }}</span
      >
    </div>
