<template>
  <div class="p-4">
    <div id="name" class="flex flex-col items-start justify-start mt-8">
      ...
      />
      <span
        v-show="errors[`name`] === true && selectedLang === defaultLang"
    ...
    <div class="flex flex-col items-start justify-start mt-8">
      <label
        ...
        >{{ $t('productDetails.currencyPosition') }}</label
      >
      <select
        id="dropdown"
    ...
    <div class="flex flex-col items-start mt-8">
      <label :class="sectionsStyle.fieldLabel">{{
        $t('productDetails.videoControls')
      }}</label>
      <span class="flex text-start text-xs mb-1 text-gray-700">{{
        $t('productDetails.videoOptions')
      }}</span>
    </div>
    ...
    <div id="imageFit" class="flex flex-col items-start justify-start mt-8">
      <label for="dropdown" class="flex section-module-upload-media-label">{{
        $t('forms.imageFit')
      }}</label>
      <span class="text-start text-xs text-gray-700 mb-1">{{
        $t('productDetails.imageFitDesc')
      }}</span>
      <select
        id="dropdown"
    ...
    <LazyFormLink
      ...
      @update:other-link="
        (val) => {
          settings[0].ctaLink[selectedLang] = val
        }
      "
    ...
    <div id="media" class="flex flex-col mt-4">
      <LazySectionsFormsFieldSets
        :array-data-pop="private_data.productMedias"
        :fieldset-group="'productDetails'"
        :legend-label="$t('forms.media')"
        @array-updated="(data) => (private_data.productMedias = data)"
        @remove-fieldset="(object, idx) => removeProductMediaBlock(idx)"
      >
        <template #default="{ object, idx }">
          <div class="my-4">
            <label class="flex section-module-upload-media-label">{{
              $t('productDetails.mediaType')
            }}</label>
          ...
          <div v-if="object.mediaType === 'image'">
            <div class="mb-4">
              <LazyMediasUploadMedia
                :media-label="
                  idx === 0 ? $t('forms.media') + '*' : $t('forms.media')
                "
          ...
          <div
            v-else-if="object.mediaType === 'video'"
          ...
          >
            ...
            <span class="text-xs text-Gray_800">{{
              $t('productDetails.videoEmbedDesc')
            }}</span>
            <input
...
export default {
  ...
  watch: {
    ...
    selectedMedia(mediaObject) {
      ...
      const mediaIndex = this.private_data.medias.findIndex(
        (media) => media.media_id === mediaObject.id,
      )
      if (mediaIndex !== -1) {
  ...
  methods: {
    ...
    removeProductMedia(idx) {
      try {
        const mediaIndex = this.private_data.medias.find(
          (md) =>
            this.private_data.productMedias[idx].media &&
            md.media_id === this.private_data.productMedias[idx].media.media_id,
        )
        this.private_data.medias = this.private_data.medias.filter(
          (img, i) => mediaIndex !== i,
        )
        this.private_data.productMedias[idx].media = {
          media_id: '',
    ...
    removeProductMediaBlock(idx) {
      if (this.private_data.productMedias.length > 1) {
        this.private_data.productMedias =
          this.private_data.productMedias.filter((img, i) => idx !== i)
      }
      try {
        const mediaIndex = this.private_data.medias.find(
          (md) =>
    ...
    validate() {
      this.private_data.productMedias = this.private_data.productMedias.filter(
        (item, index) => {
      ...
      this.settings[0].productMedias = this.settings[0].productMedias.filter(
        (item, index) => {
          if (index === 0) return true
          if (item.mediaType === 'image') {
            return !!item.media?.url // keep only if media.url is non-empty
      ...
      if (this.settings[0].productMedias[0].mediaType === 'image') {
        if (
          !this.settings[0].productMedias[0].media ||
          !this.settings[0].productMedias[0].media.url
        ) {
        ...
        if (
          !this.settings[0].productMedias[0].video ||
          !this.settings[0].productMedias[0].video.url
        ) {
          this.errors.media = true
          valid = false
        }
