    ...
    <div
      ...
      <div class="p-6">
        <div class="flex justify-between items-center mb-6">
          <h2 class="text-xl font-bold">{{ $t('ServicePackages.cart') || 'Your Bookings' }}</h2>
        ...
        <div v-if="cart.length === 0" class="text-center py-8">
          ...
          <p class="text-gray-600">{{ $t('ServicePackages.emptyCart') || 'You have no bookings' }}</p>
          <div @click="$emit('close')"
            class="mt-4 px-4 py-2 bg-blue-600 text-white rounded-lg font-semibold hover:bg-blue-700">
            {{ $t('ServicePackages.startBooking') || 'Browse Services' }}
          </div>
        </div>
        ...
        <div v-else>
          <div v-for="(item, index) in cart" :key="`cart-item-${index}`" class="border-b py-4">
            <div class="flex justify-between">
              <div class="flex-1">
                ...
                <!-- Date and Time Display for Services -->
                <div v-if="item.date || item.timeSlot" class="mt-1 mb-2 text-sm">
                  <div v-if="item.date" class="text-gray-700">
                    <span class="font-medium">{{ $t('ServicePackages.date') || 'Date' }}:</span> {{ formatDate(item.date) }}
                  </div>
                  <div v-if="item.timeSlot" class="text-gray-700">
                    <span class="font-medium">{{ $t('ServicePackages.time') || 'Time' }}:</span> {{ item.timeSlot }}
                  </div>
                </div>
                
                <div class="flex items-center justify-between mt-2">
                  <div class="flex items-center">
                    <span class="text-gray-600 mr-2">{{ $t('ServicePackages.qty') || 'Qty' }}:</span>
                    <div class="flex items-center">
          ...
          <div class="mt-4 pt-4 border-t">
            <div class="flex justify-between mb-2">
              <span>{{ $t('ServicePackages.subtotal') || 'Subtotal' }}:</span>
              <span class="font-medium">{{ currencySymbol }}{{ formatPrice(subtotal) }}</span>
            </div>
            
            <!-- Service fee -->
            <div v-if="enableServiceFee" class="flex justify-between mb-2">
              <span>{{ $t('ServicePackages.serviceFee') || 'Service Fee' }} ({{ (serviceFeeRate * 100).toFixed(2) }}%):</span>
              <span class="font-medium">{{ currencySymbol }}{{ formatPrice(serviceFee) }}</span>
            </div>
            
            <!-- Only show tax if it's enabled -->
            <div v-if="enableTax" class="flex justify-between mb-4">
              <span>{{ $t('ServicePackages.tax') || 'Tax' }} ({{ (taxRate * 100).toFixed(2) }}%):</span>
              <span class="font-medium">{{ currencySymbol }}{{ formatPrice(tax) }}</span>
            </div>
            
            <div class="flex justify-between text-lg font-bold">
              <span>{{ $t('ServicePackages.total') || 'Total' }}:</span>
          ...
          <div @click="$emit('checkout')"
            class="w-full py-3 bg-blue-600 text-white rounded-lg font-semibold hover:bg-blue-700 mt-6 flex justify-center items-center">
            {{ $t('ServicePackages.checkout') || 'Complete Booking' }}
          </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  computed: {
    ...
    serviceFee() {
      return this.enableServiceFee ? this.subtotal * this.serviceFeeRate : 0;
    },
    tax() {
