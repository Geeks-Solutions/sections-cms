    ...
    >
      <div class="relative flex flex-col md:flex-row md:w-full">
        ...
        <div class="md:w-1/2 p-6">
          ...
          <div
            v-if="isService && item.details && item.details.length > 0"
            class="mb-4"
          >
            <ul
              class="feature-list overflow-y-auto max-h-[61px] lg:max-h-[87px]"
            >
              <li
          ...
          <div v-else-if="isService" class="mb-4">
            <ul
              class="feature-list overflow-y-auto max-h-[61px] lg:max-h-[200px]"
            >
              <li
                v-if="item.duration && item.duration[lang] != ''"
                class="feature-list-item"
              >
                {{
              ...
              <li
                v-for="(feature, index) in parseFeatures(
                  item.description[lang],
                )"
                :key="index"
                class="feature-list-item"
          ...
          <div class="flex items-center mb-2">
            <label class="input-label qty-label mr-4"
              >{{
                isService
                  ? $t('ServicePackages.quantity')
                  : $t('RestaurantMenu.quantity')
              }}:</label
            >
            <div class="custom-quantity-control flex items-center">
              ...
              <input
                ...
                class="item-qty-value w-12 text-center"
                readonly
              />
              <button
          ...
          <div class="mb-4">
            <label class="input-label special-instructions block mb-2"
              >{{
                isService
                  ? $t('ServicePackages.specialRequests')
                  : $t('RestaurantMenu.specialInstructions')
              }}:</label
            >
            <textarea
              :value="notes"
              ...
              :placeholder="
                isService
                  ? $t('ServicePackages.specialRequestsPlaceholder')
                  : $t('RestaurantMenu.specialInstructionsPlaceholder')
              "
            ></textarea>
          </div>
          ...
          <button
            ...
            :class="{
              'add-to-cart-service': isService,
              'add-to-cart-restaurant': !isService,
            }"
          >
            {{
              isService
                ? $t('ServicePackages.addToCart')
                : $t('RestaurantMenu.addToCart')
            }}
          </button>
        </div>
      </div>
