<template>
  <div class="modal-overlay fixed inset-0 z-50 flex items-start md:items-center justify-center overflow-y-auto">
    <div class="item-modal-content rounded-lg shadow-xl max-w-2xl w-full my-4 md:my-0"
      ...
      <div class="relative flex flex-col md:flex-row md:w-full">
        ...
        <!-- Item image -->
        <div v-if="item && item.image && item.image.url" class="md:w-1/2"
          :class="isService ? 'md:h-unset' : 'md:h-unset'">
          <GUniversalViewer :src="item.image.url" :alt="item.image.seo_tag || (item.name && item.name[lang])"
            :type="item.image.metadata?.type || 'image'"
            class="modal-image w-full h-full object-cover rounded-t-lg md:rounded-t-none md:rounded-bl-lg md:rounded-tl-lg"
            width="400" height="160" :modifiers="{
              width: 400,
              height: 160,
              fit: 'cover'
            }" />
        ...
        <div class="md:w-1/2 p-6">
          ...
          <div class="flex flex-col justify-between items-start mb-2">
            ...
            <div class="flex items-center ">
              ...
              <span v-if="isService && item.hasDiscount" class="item-price-regular line-through mr-2">{{
                currencySymbol }}{{ formatPrice(item.price, currencySymbol) }}</span>
              <!-- Current price -->
              <span class="modal-item-price mr-2">{{ currencySymbol }}{{ formatPrice(displayPrice, currencySymbol)
                }}</span>
          ...
          <div v-if="!isService && item.hasVariants && item.variants && item.variants.length > 0" class="mb-4">
            <label class="input-label block mb-2">{{ $t('RestaurantMenu.selectVariant') }}:</label>
            <div class="flex gap-2 overflow-x-auto pb-2" style="-webkit-overflow-scrolling: touch;">
              <button v-for="variant in item.variants" :key="variant.id" @click="$emit('select-variant', variant)"
                class="variant-button px-4 py-2 rounded-lg border-2 transition-all" :class="{
                  'variant-selected': selectedVariant && selectedVariant.id === variant.id,
                  'variant-unselected': !selectedVariant || selectedVariant.id !== variant.id
                }">
                <span class="font-medium">{{ variant.name[lang] }}</span>
                <span class="ml-2 text-sm">{{ currencySymbol }}{{ formatPrice(variant.price, currencySymbol) }}</span>
              </button>
            </div>
          </div>
          ...
          <div class="mb-4">
            <label class="input-label special-instructions block mb-2">{{ isService ?
              $t('ServicePackages.specialRequests') :
          ...
          <button v-if="showAddToCart" @click="addToCart"
