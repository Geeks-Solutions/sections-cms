    ...
    <div class="absolute right-0 top-0 h-full cart-sidebar w-full max-w-md overflow-y-auto shadow-xl" @click.stop>
      <div class="cart-container">
        <div class="cart-header flex justify-between items-center">
          <h2 class="cart-title">{{ isService ? $t('ServicePackages.yourCart') : $t('RestaurantMenu.cart') }}</h2>
        ...
        <div v-if="cart.length === 0" class="empty-cart-message flex flex-col items-center">
          ...
          <p>{{ isService ? $t('ServicePackages.emptyCart') : $t('RestaurantMenu.emptyCart') }}</p>
          <div @click="$emit('close')" class="empty-cart-button"
            :class="isService ? 'service-theme' : 'restaurant-theme'">
            {{ isService ? $t('ServicePackages.startBooking') : $t('RestaurantMenu.startOrdering') }}
          </div>
        </div>
        ...
        <div v-else class="cart-items-container flex flex-col gap-4">
          ...
          <div class="cart-summary">
            ...
            <div v-if="enableTax || (isService && enableServiceFee)" class="cart-subtotal mb-3 flex justify-between">
              <h3 class="cart-subtotal-label">{{ isService ? $t('ServicePackages.subtotal') :
                $t('RestaurantMenu.subtotal') }}:</h3>
              <span class="cart-subtotal-value">{{ currencySymbol }}{{ formatPrice(subtotal, currencySymbol) }}</span>
            </div>
            ...
            <div v-if="isService && enableServiceFee" class="cart-service-fee mb-3 flex justify-between">
              <h3 class="cart-summary-label">{{ $t('ServicePackages.serviceFee') }} ({{ (serviceFeeRate *
                100).toFixed(2) }}%):</h3>
              <span class="cart-summary-value">{{ currencySymbol }}{{ formatPrice(serviceFee, currencySymbol) }}</span>
            </div>

            <!-- Tax (if enabled) -->
            <div v-if="enableTax" class="cart-tax mb-3 flex justify-between">
              <h3 class="cart-summary-label">{{ isService ? $t('ServicePackages.tax') : $t('RestaurantMenu.tax') }} ({{
                (taxRate * 100).toFixed(2) }}%):</h3>
              <span class="cart-summary-value">{{ currencySymbol }}{{ formatPrice(tax, currencySymbol) }}</span>
            </div>

            <!-- Total -->
            <div class="cart-total my-3 flex justify-between items-center">
              <h3 class="cart-total-label">{{ isService ? $t('ServicePackages.total') : $t('RestaurantMenu.total') }}:
              </h3>
              <h3 class="cart-total-value">{{ currencySymbol }}{{ formatPrice(total, currencySymbol) }}</h3>
            </div>
          </div>

          <!-- Branch Selector (if branches enabled) -->
          <div v-if="enableBranches && branches.length > 0" class="mb-4">
            <label class="block text-sm font-medium mb-2">{{ $t('RestaurantMenu.selectBranch') }}:</label>
            <select v-model="selectedBranch"
              class="w-full p-3 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
              <option :value="null" disabled>{{ $t('RestaurantMenu.selectBranch') }}</option>
              <option v-for="branch in branches" :key="branch.id" :value="branch">
                {{ branch.name[lang] || branch.name.en || 'Branch' }}
              </option>
            </select>
            <span v-if="!selectedBranch" class="text-xs text-red-600 mt-1 block">
          ...
          <div v-if="cart.length > 0" class="checkout-buttons w-full">
            ...
            <a v-if="whatsappEnabled || enableBranches" :href="(enableBranches && !selectedBranch) ? '#' : whatsappUrl"
              @click="(e) => { if (enableBranches && !selectedBranch) e.preventDefault() }" target="_blank"
              rel="noopener" class="whatsapp-checkout-button w-full flex items-center justify-center" :class="[
                isService ? 'service-theme' : 'restaurant-theme',
                { 'opacity-50 cursor-not-allowed': enableBranches && !selectedBranch }
              ]">
              ...
              {{ isService ? $t('ServicePackages.orderViaWhatsApp') : $t('RestaurantMenu.orderViaWhatsApp') }}
            </a>
          </div>
        </div>
      </div>
...
const getItemName = (item) => {
  ...
  if (item.variant && item.variant.name) {
    const variantName = typeof item.variant.name === 'object'
