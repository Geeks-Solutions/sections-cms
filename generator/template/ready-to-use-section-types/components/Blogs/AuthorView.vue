<template>
  <div v-if="sectionRenderData" class="flex flex-col">
    <div class="flex flex-col items-center bg-white px-4 md:px-20 pb-24">
      <div class="flex flex-col md:flex-row justify-between border-t border-Gray_200 pt-6 w-full md:w-720px">
        <div v-if="sectionRenderData.profile_image || sectionRenderData.full_name || sectionRenderData.biography" class="flex flex-row gap-4">
          ...
          <div class="flex flex-col">
            <div v-if="sectionRenderData.full_name" class="text-lg text-Gray_900 font-semibold">{{ sectionRenderData.full_name }}</div>
            <div v-if="sectionRenderData.biography && sectionRenderData.biography[lang]" class="text-Gray_600">{{ sectionRenderData.biography[lang] }}</div>
          </div>
        </div>
        <div v-else></div>
        <div class="flex flex-row gap-3 md:pb-4">
          <div class="flex flex-row rounded-lg py-2.5 px-4 gap-2 bg-white border border-Gray_300 shadow cursor-pointer" @click="copyArticleLink">
            <IconsCopy />
            <div class="text-sm font-semibold text-Gray_700">{{ $t('copyLink') }}</div>
          </div>
