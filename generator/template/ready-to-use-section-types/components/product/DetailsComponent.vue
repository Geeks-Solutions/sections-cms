<template>
  <div v-if="product" class="product-details-wrapper main-wrapper py-2.5">
    <div
      class="flex flex-col md:flex-row justify-center w-full gap-6 row-wrapper"
    >
      <div class="flex flex-col md:w-1/2 medias-block items-center">
        <div
          v-if="
            ...
            (mediaPreview.mediaType === 'image' ||
              mediaPreview.mediaType === 'imageType')
          "
          class="image-preview md:justify-items-start justify-items-center md:cursor-pointer"
          @click="openPreview(mediaPreview)"
        >
          <GUniversalViewer
        ...
        <div
          v-if="mediaPreview && mediaPreview.mediaType === 'video'"
          class="video-preview md:w-[560px] h-[280px] md:h-[310px] content-center"
        >
          <LazyYoutube
            v-if="mediaPreview.video && mediaPreview.video.url"
        ...
        <div
          v-if="product.productMedias && product.productMedias.length > 1"
          class="flex flex-row gap-3 overflow-x-scroll max-w-full mt-4 mx-1.5 medias-list"
        >
          <div
            v-for="(mediaObject, idx) in product.productMedias"
            :key="`product-image-${idx}`"
            class="relative medias-list-item"
          >
            <div
              v-if="mediaObject && mediaObject.mediaType === 'image'"
              class="medias-list-image"
            >
              <GUniversalViewer
                v-if="
                  (mediaObject.media && mediaObject.media.thumbnail_url) ||
                  (mediaObject.media && mediaObject.media.url)
                "
                :src="
                  mediaObject.media.metadata?.type === 'lottie'
                    ? mediaObject.media.url
                    : mediaObject.media.thumbnail_url
                "
                ...
                :class="
                  mediaPreview.media &&
                  mediaPreview.media?.url === mediaObject.media.url
                    ? 'p-1 border-2 border-black shadow-md'
                    : ''
                "
                width="300"
                height="300"
            ...
            <div
              v-if="mediaObject && mediaObject.mediaType === 'video'"
              class="medias-list-video w-[60px] min-w-[60px] h-[60px] content-center pointer-events-auto md:cursor-pointer"
              :class="
                mediaPreview.video &&
                mediaPreview.video?.url === mediaObject.video?.url
                  ? 'px-1 pt-1 pb-2.5 border-2 border-black shadow-md'
                  : ''
              "
              @click="mediaPreview = mediaObject"
            >
              <LazyYoutube
                v-if="mediaObject.video && mediaObject.video.url"
                :src="mediaObject.video.url"
                max-width="60px"
      ...
      <div
        class="flex flex-col md:w-1/2 rounded-lg px-8 justify-between details-main-wrapper"
      >
        <div class="flex flex-col gap-3 pb-6 details-wrapper">
          <h2 class="product-name">{{ product.name[lang] }}</h2>

          <div
            class="flex self-start price-block"
            :class="
              product.currencyPosition && product.currencyPosition === 'right'
                ? 'flex-row-reverse'
                : 'flex-row'
            "
          >
          ...
          <gWysiwygContent
            v-if="
              ...
              product.description[lang] !== '<p><br></p>'
            "
            tag="p"
            :wrapper-classes="['desc-wrapper']"
            :classes="`p-0 h-auto pt-4`"
            :html-content="product.description[lang]"
          />
        </div>

        <global-link
          v-if="product.ctaLabel && product.ctaLabel[lang]"
          :link="
            product.sectionsPage[lang] === 'other'
              ? product.ctaLink
              : product.sectionsPage && product.sectionsPage[lang]
                ? {
                    ...product.sectionsPage,
                    en: '/' + product.sectionsPage.en,
                    fr: '/' + product.sectionsPage.fr,
                  }
                : '#'
          "
          :lang="lang"
          ...
          :class="
            product.productMedias && product.productMedias.length > 1
              ? 'btn-bottom-align'
              : ''
          "
        >
    ...
    >
      ...
      >
        ...
        >
          <GUniversalViewer
            v-if="
              selectedImage && selectedImage.media && selectedImage.media.url
            "
...
const computedVideoUrl = computed(() => {
  if (
    props.product &&
    mediaPreview.value.video?.url &&
    isYouTubeLink(mediaPreview.value.video?.url)
  ) {
...
@container product-details-wrapper (max-width: 768px) {
  ...
  .product-details-wrapper .medias-block,
  .product-details-wrapper .details-main-wrapper {
    width: 100% !important;
  }
