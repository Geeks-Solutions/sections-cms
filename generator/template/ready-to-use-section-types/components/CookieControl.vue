  ...
  <div class="cookieControl">
      ...
      >
        <div class="cookieControl__BarContainer">
          ...
          <div class="cookieControl__BarButtons">
            <button
              v-if="cookies.acceptNecessary"
              class="button-selector"
              @click="cookies.acceptNecessary"
            >
              {{ $t('cookieConsent.necessary') }}
            </button>
            <button class="button-selector" @click="cookies.modal = true">
    ...
    <transition name="cookieControl__Modal">
      <div v-if="cookies.modal" class="cookieControl__Modal">
        <p v-if="!saved" class="cookieControl__ModalUnsaved">
          {{ $t('cookieConsent.unsaved') }}
        </p>
        <div class="cookieControl__ModalContent">
          <div>
                  ...
                  <li v-for="cookie in cookies[type]" :key="cookie.id">
                    <div class="cookieControl__ModalInputWrapper">
                      ...
                      <input
                        ...
                        type="checkbox"
                        :checked="
                          (useCookie('cookie_control_consent').value !==
                            undefined &&
                            useCookie('cookie_control_consent').value ===
                              true &&
                            useCookie('cookie_control_enabled_cookies').value &&
                            useCookie(
                              'cookie_control_enabled_cookies',
                            ).value.includes(cookie.identifier)) ||
                          (useCookie('cookie_control_consent').value ===
                            undefined &&
                      ...
                      />
                      <label
                        :for="getCookieFirstName(cookie.name)"
                        v-html="cookie.name"
                      />
            ...
            <div class="cookieControl__ModalButtons">
              <button
                class="button-selector shadowed"
              ...
              <button
                class="button-selector shadowed"
                @click="setConsent({ declineAll: true, consent: true })"
              >
                {{ $t('cookieConsent.declineAll') }}
              </button>
              <button class="button-selector" @click="setConsent">
                {{ $t('cookieConsent.acceptAll') }}
              </button>
            </div>
          </div>
...
export default {
      ...
      cookies: {
        ...
        optional: [
          ...
          },
          {
            ...
            async: true,
            cookies: [
              '_hjIncludedInSessionSample_XXXXXXX',
              '_hjFirstSeen',
  ...
  beforeMount() {
    try {
      ...
      if (
        useCookie('cookie_control_enabled_cookies').value !== undefined &&
        useCookie('cookie_control_enabled_cookies').value.length !== 0
      ) {
        this.optionalCookies.forEach((c) => {
          for (const cookieKey of Object.keys(allCookies)) {
            if (
              cookieKey.includes(c.identifier) &&
              !useCookie('cookie_control_enabled_cookies').value.includes(
                c.identifier,
              )
            ) {
        ...
        useCookie('cookie_control_consent').value === false ||
        useCookie('cookie_control_enabled_cookies').value.length === 0
      ) {
        this.optionalCookies.forEach((c) => {
          for (const cookieKey of Object.keys(allCookies)) {
  ...
  methods: {
    ...
    setConsent({ type, consent = true, reload = true, declineAll = false }) {
      useCookie('cookie_control_consent', {
      ...
      const enabledCookies = declineAll
        ...
        : type === 'partial' && consent
          ...
          : [...this.optionalCookies.map((c) => c.identifier)]
      useCookie('cookie_control_enabled_cookies', {
        expires: this.expirationDate,
