<template>
  <div
    v-if="selectedLayout === 'thumbnails'"
    class="space-y-4 thumbnails-wrapper"
  >
    <div
    ...
    >
      <GUniversalViewer
        ...
        :alt="
          images[currentSlide].media.seo_tag
            ? images[currentSlide].media.seo_tag
            : ''
        "
        ...
        :class="{
          mobileHidden:
            images[currentSlide].mediaMobile &&
            images[currentSlide].mediaMobile.url,
        }"
        :style="
      ...
      <GUniversalViewer
        ...
        :alt="
          images[currentSlide].mediaMobile.seo_tag
            ? images[currentSlide].mediaMobile.seo_tag
            : ''
        "
    ...
    <div class="relative">
      <div class="overflow-x-hidden p-2">
        <div
          ...
          :class="
            canScrollThumbnailsLeft === true || canScrollRight === true
              ? ''
              : 'justify-center'
          "
        ...
        >
          ...
          >
            <GUniversalViewer
              ...
              :class="[
                { 'opacity-70': currentSlide !== imageIdx },
...
export default {
  ...
  watch: {
    ...
    thumbsWrapperWidth: {
      handler() {
        if (
          this.thumbsWrapperWidth !== null &&
          this.thumbsWrapperWidth !== undefined
        ) {
          this.canScrollThumbnailsRight()
        }
      },
    ...
    canScrollThumbnailsRight() {
      this.$nextTick(() => {
        if (process.client) {
          ...
          const containerWidth = this.thumbsWrapperWidth
            ? this.thumbsWrapperWidth - 96
            : 0
          const totalWidth = this.images.length * (thumbnailWidth + 8)
          this.canScrollRight =
            this.thumbnailScroll < totalWidth - containerWidth
