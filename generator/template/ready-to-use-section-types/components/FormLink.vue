<template>
  <div class="text-start">
    <div class="flex flex-col items-start justify-start mt-8">
      ...
      <label v-if="sectionsPagesLabel" class="mr-4 font-bold">{{
        sectionsPagesLabel
      }}</label>
      <span
        v-if="errorLoadingPagePaths"
        class="flex text-start text-xs text-error pb-2"
        >{{ $t('SectionsPagesError') }}</span
      >
      <gAutoComplete
        ...
        :filter-options="[
          ...sectionsPages,
          { id: 'other', page: 'Other', path: 'other' },
        ]"
    ...
    <div
      v-if="selectedSectionsPage === 'other'"
      class="flex flex-col items-start justify-start mt-8"
    >
      <label v-if="otherLinkLabel" class="mr-4 font-medium">{{
        otherLinkLabel
      }}</label>
      ...
      <input
        ...
        @input="
          (evt) => {
            $emit('update:otherLink', evt.target.value)
          }
        "
      />
    </div>

    <span v-show="linkError === true" class="text-error text-sm pt-2 pl-2">{{
      $t('forms.requiredField')
    }}</span>

    <div class="my-4">
      <label
        v-if="linkTargetLabel"
        class="flex section-module-upload-media-label"
      ...
      <div class="select-style-chooser">
        <gAutoComplete
          ...
          :filter-options="[
            { key: '_self', value: $t('forms.selfTarget') },
            { key: '_blank', value: $t('forms.blankTarget') },
          ]"
          :filter-searchable="false"
          :close-on-select="true"
          :filter-clearable="true"
